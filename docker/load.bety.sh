...
if ! psql ${PG_OPT} -U ${OWNER} -lqt | cut -d \| -f 1 | grep -w "${DATABASE}" > /dev/null ; then
...
if [ "${CREATE}" == "YES" ]; then
  ...
  psql ${PG_OPT} -U ${OWNER} -q -d "${DATABASE}" -c "DROP SCHEMA public CASCADE"
  psql ${PG_OPT} -U ${OWNER} -q -d "${DATABASE}" -c "CREATE SCHEMA public"

  # following commands require superuser abilities
  psql ${PG_OPT} -U ${OWNER} -d "${DATABASE}" -c 'CREATE EXTENSION Postgis;'
  psql ${PG_OPT} -U ${OWNER} -d "${DATABASE}" -c "GRANT ALL ON ALL TABLES IN SCHEMA public TO ${OWNER};"
