      ...
      if counties.length > 0
        counties.each do |county|
          ...
          if county.county_boundaries.length == 1
            canvas.g do |_canvas|
             # p "circ: #{county.id}"
              _canvas.circle(1,county.county_boundries[0]["zoom#{zoom}x"],county.county_boundaries[0]["zoom#{zoom}y"]).styles(:fill=>"#{color}")
            end
          else
