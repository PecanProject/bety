...
public class HtmlTablePage extends HtmlFormatter {
    ...
    public WriteStats write(Database db, Table table, boolean hasOrphans, File outputDir, WriteStats stats, LineWriter out) throws IOException {
        ...
        writeHeader(table, hasImplied, out);///write checkboxes
        writeLegend(out);
        out.writeln("<div class='sixteen colums'>");        
        writeMainTable(table, out);
        out.writeln("</div>"); 
        if(nocomment)
            out.writeln("<script>$('.comment').hide()</script>");// if no comment hide comment
        if(nofkeys)
            out.writeln("<script>$('.fkey').hide()</script>");// if no fkeys hide fkeys
 ...
 //////CHANGED      // html.writeln("<form name='options' action=''>");
        if (hasImplied) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        html.writeln(" <label for='showComments'><input type=checkbox " + (nocomment ? " ":"checked " ) + "id='showComments'>Comments</label>");
    ...
    private void writeRelatives(TableColumn baseRelative, boolean dumpParents, String path, boolean even, LineWriter out) throws IOException {
        ...
        /*if (numColumns > 0) {
            out.newLine();
            out.writeln("  <table class='fkey' border='0' width='100%' cellspacing='0' cellpadding='0'>");
        }*/
