class UsersController < ApplicationController
  ...
  def create
    ...
    if params[:user][:page_access_level].to_i != 4 or params[:user][:access_level].to_i != 3
      ...
      xml += "<subject>Access request for #{@user.name.to_s} (#{@user.login.to_s})</subject>"
      xml += "<description>https://www.betydb.org/users/#{@user.id}/edit\n"
      xml += "\n"
      xml += "#{@user.name.to_s} (#{@user.login.to_s}) has requested a non-default access level.\n"
      ...
      xml += "\n"
      xml += "Reason: #{params[:access_level_reason].to_s}\n"
      xml += "</description>"
      xml += "</issue>'"
